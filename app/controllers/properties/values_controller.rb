class Properties::ValuesController < GenericController
  ...
  def index
    if current_user&.admin_less? && admin_view?
      @list_pages = Admin::ListPages.new do |l|
        ...
        l.list = Properties::Value.ordering.page(params[:page])
      end
  ...
  def set_actions
    @admin_panel = Admin::Panel::Base.new(@value || ::Properties::Value)
